...
void lcd_ini(void) {
    ...
    LCD->GCR =
            ...
            LCD_GCR_RVTRIM(0x00) |
            //Set RVTRIM to 0, irrelevant as voltage regulator is disabled, but setting it to a known state
            LCD_GCR_CPSEL_MASK |   //Set LCD_GCR_CPSEL to use capacitor charge pump
            LCD_GCR_LADJ(0x03) |
            //Set LCD_GCR_LADJ to 11, slow clock rate = lower power, but higher load capacitance on the LCD requires higher clock speed
            ...
            LCD_GCR_FFR_MASK |
            //Set LCD_GCR_FFR, allow an LCD Frame Frequency of 46.6Hz to 146.2Hz.  Disable to change range to 23.3Hz to 73.1Hz
            ...
            LCD_GCR_LCLK(0x04) |
            //Set LCD_GCR_LCLK to 111, LCD Clock prescaler where LCD controller frame frequency = LCD clock/((DUTY  |  1) x 8 x (4 | LCLK[2:0]) x Y), where Y = 2, 2, 3, 3, 4, 5, 8, 16 chosen by module duty cycle config
    ...
    LCD->AR =
            ...
            LCD_AR_BRATE(
                    0x00);   //Set LCD_SEG_AR_BRATE to 000.  Frequency of blink is determined by LCD clock/(2^(12 + BRATE))
...
extern int My_Div2(int a, int b);
...
void lcd_display_dec(uint16_t value) {
    if (value > 9999) {
        ...
        lcd_set(My_Div2((value - (My_Div2(value, 1000)) * 100), 10), 3);
        lcd_set(value - (My_Div2(value, 1000)) * 10, 4);
    }
}
